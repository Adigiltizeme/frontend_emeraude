  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
        <h1 style={{ color: 'var(--color-primary-900)' }}>{t('admin.dashboard.title')}</h1>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <span style={{ fontWeight: '500', color: 'var(--color-neutral-700)' }}>{t('admin.dashboard.hello', { name: user?.firstName || 'Admin' })}</span>
          <button onClick={handleLogout} className="btn btn-outline" style={{ padding: '0.5rem 1rem' }}>{t('admin.dashboard.logout')}</button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
        <div style={{ backgroundColor: 'var(--color-white)', padding: '2rem', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--color-neutral-200)' }}>
          <h3 style={{ marginBottom: '1rem', color: 'var(--color-primary-800)' }}>{t('admin.dashboard.manageProjects')}</h3>
          <p style={{ color: 'var(--color-neutral-600)', marginBottom: '1.5rem' }}>{t('admin.dashboard.manageProjectsDesc')}</p>
          <button onClick={() => navigate('/admin/projects/new')} className="btn btn-primary" style={{ width: '100%' }}>{t('admin.projectForm.createTitle')}</button>
        </div>
      ...
      <div style={{ marginTop: '3rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <h2 style={{ color: 'var(--color-primary-900)' }}>{t('admin.dashboard.projectsTitle')}</h2>
