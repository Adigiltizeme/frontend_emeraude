...
const EditProject = () => {
  ...
  const handleSubmit = async (projectData: any, imageFile: File | null) => {
    ...
    try {
      const projectResponse = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5050'}/projects/${id}`, {
        method: 'PATCH',
        headers: {
      ...
      if (imageFile) {
        ...
        const imageResponse = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5050'}/projects/${id}/image`, {
          method: 'POST',
  ...
  return (
    <div style={{ padding: '3rem 2rem', maxWidth: '900px', margin: '0 auto', backgroundColor: 'var(--color-neutral-100)', minHeight: '100vh' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ color: '#0f172a', fontSize: '2rem', marginBottom: '0.5rem' }}>Modifier le projet</h1>
          <p style={{ color: '#475569' }}>Mettez à jour les informations du projet</p>
        </div>
